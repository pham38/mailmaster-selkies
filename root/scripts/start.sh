...
# start Mailmaster application in the background if exists and auto-start enabled
if [ "$AUTO_START_Mailmaster" = "true" ]; then
    if [ -f /opt/mailmaster/mailmaster ]; then nohup /opt/mailmaster/mailmaster > /dev/null 2>&1 & fi
fi
